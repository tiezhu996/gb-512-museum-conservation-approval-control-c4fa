...
博物馆文物风险、保护方案、材料检测和阶段审批平台。项目采用前后端分离和明确的领域分层，重点保证状态迁移、RBAC、审计日志、请求追踪与限流在各层保持一致。
...
- Web 工作台：http://127.0.0.1:18512
- 后端健康检查：http://127.0.0.1:19512/healthz
- 后端 API：http://127.0.0.1:19512/api
...
```bash
token=$(curl -sS -X POST http://127.0.0.1:19512/api/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"username":"admin","password":"Admin123!"}' | jq -r '.data.token')

curl -sS http://127.0.0.1:19512/api/overview \
