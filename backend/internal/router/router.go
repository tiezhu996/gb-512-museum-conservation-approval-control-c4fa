...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	approvalGateService := service.NewApprovalGateService(treatmentPlanRepository, materialTestRepository)
	stageApprovalService := service.NewStageApprovalService(stageApprovalRepository, securityService, approvalGateService)
